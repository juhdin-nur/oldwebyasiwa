    ...
    function build_menu($table = 'dyn_menu', $type = '2')
    {
            ...
            if (is_array($menu[$i]))    // must be by construction but let's keep the errors home
            {
                if ($menu[$i]['showonadmin'] && $menu[$i]['parent'] == 0)    // are we allowed to see this menu?
                {
                    if ($menu[$i]['is_parent'] == TRUE)
                    {
                        ...
                        $attribut1 = array("class"=>"dropdown-toggle");
                        $html_out .= "\t\t\t".'<li>'.anchor($menu[$i]['link'],'<i class="menu-icon '.$menu[$i]['icon'].'"></i><span class="menu-text">'.$this->ci->lang->line($menu[$i]['title']).'</span><b class="arrow fa fa-angle-down"></b></a><b class="arrow"></b>',$attribut1);
                    }
                    else if($menu[$i]['is_parent']==FALSE && $menu[$i]['parent']==0)
                    {
                        // $html_out .= "\t\t\t".'<li><a href="'.$menu[$i]['link'].'" id="'.$menu[$i]['link'].'"><i class="menu-icon '.$menu[$i]['icon'].'"></i><span class="menu-text">'.$this->ci->lang->line($menu[$i]['title']).'</span></a>';
                           $html_out .= "\t\t\t".'<li>'.anchor($menu[$i]['link'],'<i class="menu-icon '.$menu[$i]['icon'].'"></i><span class="menu-text">'.$this->ci->lang->line($menu[$i]['title']).'</span></a>');
                    }
                    else
                    {
        ...
        $html_out .= '<li class="">
                        ...
                        <ul class="submenu nav-hide" style="display: none;">
                            <li class="">
                                '.anchor("dashboard/menupg", '<span><i class="menu-icon fa fa-caret-right"></i>'.$this->ci->lang->line("menu_management")).'
                               

                                <b class="arrow"></b>
                            </li>                                   
                            <li class="">'.anchor("dashboard/cprofile", '<span><i class="menu-icon fa fa-caret-right"></i>'.$this->ci->lang->line("company_profile")).'
                                <b class="arrow"></b>
                            </li>                                   
                        </ul>
                    </li>';
        ...
        for ($i = 1; $i <= count($menu); $i++)
        {
            if ($menu[$i]['showonadmin'] && $menu[$i]['parent'] == $parent_id)    // are we allowed to see this menu?
            {
                ...
                if ($menu[$i]['is_parent'] == TRUE)
                {
                    ...
                    $attribut2 = array("class"=>"dropdown-toggle");
                    $html_out .= "\t\t\t".'<li>'.anchor($menu[$i]['link'],'<i class="menu-icon '.$menu[$i]['icon'].'"></i><span class="menu-text">'.$this->ci->lang->line($menu[$i]['title']).'</span><b class="arrow fa fa-angle-down"></b></a><b class="arrow"></b>',$attribut2);
                }
                else if($menu[$i]['is_parent']==FALSE && $menu[$i]['parent']!=0)
                {
                        $html_out .= "\t\t\t".'<li>'.anchor($menu[$i]['link'],'<i class="menu-icon '.$menu[$i]['icon'].'"></i><span class="menu-text">'.$this->ci->lang->line($menu[$i]['title']).'</span></a>');
